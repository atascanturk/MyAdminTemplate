$(document).ready(function () {
        ...
        buttons: [
            ...
            },
            {
                ...
                action: function (e, dt, node, config) {
                    $.ajax({
                        type: 'GET',
                        url: '/WNqGRjUh3JPe/User/GetAllUsers/',
    ...
    $(function () {
        const url = '/WNqGRjUh3JPe/User/Add/';
    ...
    $(function () {
        const url = '/WNqGRjUh3JPe/User/Update/';
        const placeHolderDiv = $('#modalPlaceHolder');
        $(document).on('click',
            '.btn-update',
            function (event) {
    ...
    $(function () {
        ...
        const url = '/WNqGRjUh3JPe/User/GetDetails/';
        const placeHolderDiv = $('#modalPlaceHolder');
        $(document).on('click',
            '.btn-detail',
            function (event) {
...
/* Ajax GET / Getting Roles starts from here */

    $(function () {
        const url = '/WNqGRjUh3JPe/Role/Assign/';
        const placeHolderDiv = $('#modalPlaceHolder');
        $(document).on('click',
            '.btn-assign',
            function (event) {
        ...
        placeHolderDiv.on('click',
            ...
            function (event) {
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: function (data) {
                        const userRoleAssignjaxModel = jQuery.parseJSON(data);
                        console.log(userRoleAssignjaxModel);
                        const newFormBody = $('.modal-body', userRoleAssignjaxModel.RoleAssignPartial);
                        placeHolderDiv.find('.modal-body').replaceWith(newFormBody);
                        const isValid = newFormBody.find('[name="IsValid"]').val() === 'True';
                        if (isValid) {
                            const id = userRoleAssignjaxModel.UserDto.User.Id;
                            const tableRow = $(`[name="${id}"]`);
                            placeHolderDiv.find('.modal').modal('hide');
                            
                            tableRow.attr("name", `${id}`);
                            dataTable.row(tableRow).invalidate();
                            toastr.success(`${userRoleAssignjaxModel.UserDto.Message}`, "Başarılı İşlem!");
                        } else {
                            let summaryText = "";
                            $('#validation-summary > ul > li').each(function () {
                                let text = $(this).text();
